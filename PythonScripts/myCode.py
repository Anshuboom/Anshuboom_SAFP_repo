...
class LocationSpot():
    ...
    def amInBayOfBengal(self):
        if (5 <= self.myLatitude <= 22) and (85 <= self.myLongitude <= 100 ):
            return True
        else:
            return False
        
    def amInChinaSea(self):
        if (3 <= self.myLatitude <= 20) and (109 <= self.myLongitude <= 117 ):
            return True
        else:
            return False    
    ...
    def mapN(self, df):
        ...
        mypopup = "Location:{}\nLongitude:{}\nLatitude:{}".format(self.name(), self.lng(), self.lat())
        me = self.myCoordinates
        map = folium.Map(location=me, popup= mypopup, zoom_start=5)
        ...
        if len(knownList) == 0:
            ...
            if theCountry == "UNKNOWN":
                if self.amInArabianGulf():
                    ...
                    mySea = "RED SEA"
                    regionList=getOceanSharks(mySea)
                elif self.amInBayOfBengal():
                    mySea = "BAY OF BENGAL"
                    regionList=getOceanSharks(mySea)
                elif self.amInChinaSea():
                    mySea = "SOUTH CHINA SEA"
                    regionList=getOceanSharks(mySea)
...
def makeXrow(myInputs):
    ...
    elif myPosition.amInBayOfBengal():mySea = "BAY OF BENGAL"
    elif myPosition.amInChinaSea():mySea = "SOUTH CHINA SEA"
